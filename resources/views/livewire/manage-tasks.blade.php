    ...
    <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($tasks as $task)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4">
                                            {{ implode(',', $task->group->pluck('name')->toArray()) }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4">
                                            {{-- Edit Button Action --}}
                                            <x-jet-button wire:click="confirmTaskEdit( {{ $task->id }})"
                                                class="bg-orange-500 hover:bg-orange-700">
                                                Edit
                                            </x-jet-button>
                                            @if($task->status != 'complete')
                                            <x-jet-button wire:click="completeTask( {{ $task->id }})"
                                                class="bg-orange-500 hover:bg-orange-700">
                                                Mark as Complete
                                            </x-jet-button>
                                            @endif
                                            {{-- Delete Button Action --}}
                                            <x-jet-danger-button wire:click="confirmTaskDeletion( {{ $task->id }})"
        ...
        <x-jet-confirmation-modal wire:model="confirmingTaskDeletion">
            ...
            <x-slot name="footer">
                <x-jet-secondary-button wire:click="$set('confirmingTaskDeletion', false)" wire:loading.attr="disabled">
                    {{ __('Conceal') }}
                </x-jet-secondary-button>
    
                <x-jet-danger-button class="ml-2" wire:click="deleteTask({{ $confirmingTaskDeletion }})"
                    wire:loading.attr="disabled">
                    {{ __('Delete') }}
                </x-jet-danger-button>
            </x-slot>
        ...
        <x-jet-dialog-modal wire:model="confirmingTaskUpdate">
            <x-slot name="title">
            ...
            <x-slot name="content">
                ...
                <div class="col-span-6 sm:col-span-4">
                    ...
                    <x-jet-input-error for="task.task_group" class="mt-2" />
                </div>
            ...
            <x-slot name="footer">
                <x-jet-secondary-button wire:click="$set('confirmingTaskUpdate', false)" wire:loading.attr="disabled">
                    {{ __('Cancel') }}
                </x-jet-secondary-button>
    
                <x-jet-danger-button class="ml-2" wire:click="saveTask()" wire:loading.attr="disabled">
                    {{ __('Save') }}
                </x-jet-danger-button>
            </x-slot>
        </x-jet-dialog-modal>
